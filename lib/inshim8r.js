(function (inshim8rGlobal) {
	...
	var stringifyOption = function(option) {
		switch (typeof option) {
			case "function":
				var optAsString = '' + option;
				optAsString = optAsString.trim();
				optAsString = optAsString.substring(optAsString.indexOf("{") + 1);
				optAsString = optAsString.substring(0, optAsString.lastIndexOf("}"));
    ...
    function makeApi(inshim8r) {
        ...
        inshim8r.import = function (options) {
            ...
            var prependedCode = !!options.prependedCode ? stringifyOption(options.prependedCode) : "";
            var appendedCode = !!options.appendedCode ? stringifyOption(options.appendedCode) : "";
